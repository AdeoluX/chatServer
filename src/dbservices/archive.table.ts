...
export default class PropertyRepo {
  ...
  update = async(updates: IArchiveMessage_, conditions: IArchiveMessage_): Promise<IArchiveMessage> => {
    const archive = await ArchiveMessage.findOneAndUpdate(conditions, updates, { new: true });
    return archive;
  }
}
